...
import { CREATE_PATH, FIND_BY_ID_PATH, UPDATE_PATH } from '../variables/path.js';
...
const EditPerson = () => {
  ...
  const fetchPersonById = async (id) => {
    try {
      fetchGroups();
      fetchBranches();
      const result = await axiosHelper.be1.get(`${FIND_BY_ID_PATH}?i=${id}`);
  ...
  const submitData = async () => {
    try {
      ...
      await axiosHelper.be1.patch(`${UPDATE_PATH}/${id}`, data);
      // resetForm();
      showSuccessAlert('');
    } catch (error) {
  ...
  const handleCancel = () => {
    navigate.push(`${location.pathname.replace('/edit', '')}`);
  };
  ...
  const initForm = (data) => {
    ...
    setDifferentName(data?.differentName);
    setDateOfBirth(data?.dateOfBirth);
    setCic(data?.cic);
    setGroup(data?.group?._id);
    setBranch(data?.branch?._id);
    setGenNum(data?.genNum);
    ...
    setSelectedMother(data?.mother);
    setSpouses(data?.spouses);
    setChildren(data?.children);
    setStepFathers(data?.stepFathers);
    setStepMothers(data?.stepMothers);
  };
