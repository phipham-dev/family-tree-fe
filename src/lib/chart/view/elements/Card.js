...
export function Card(props) {
  ...
  return function ({ node, d }) {
    ...
    const unknown_lbl = props.cardEditForm ? "ADD" : "Không rõ"; // phi
    const mini_tree = () =>
      !d.data.to_add && show_mini_tree
        ? MiniTree({ d, card_dim }).template
        : "";
    const card_body_outline = () =>
      CardBodyOutline({ d, card_dim, is_new: d.data.to_add }).template;
    const card_body = () =>
      !d.data.to_add
        ? CardBody({ d, card_dim, card_display: props.card_display }).template
        : CardBodyAddNew({
            d,
            card_dim,
    ...
    const card_image = () =>
      ...
      edit_icon = () =>
        !d.data.to_add && props.cardEditForm
          ? PencilIcon({ card_dim, x: card_dim.w - 46, y: card_dim.h - 20 })
              .template
          : "",
      add_icon = () =>
        !d.data.to_add && props.cardEditForm
          ? PlusIcon({ card_dim, x: card_dim.w - 26, y: card_dim.h - 20 })
              .template
          : "",
